...
namespace Krisp.BackEnd
{
	public class KrispAwsSDK : RestClient, IKrispSDK
	{
			...
			: base(baseUrl)
		{
			...
			this.AddDefaultHeader("KrispAuthorization", stoken);
			this.AddDefaultHeader("User-Agent", BackEndHelper.GetHTTPUserAgent());
			this.AddDefaultHeader("KRISP_HEADER_APP", "win-" + EnvHelper.KrispVersion.ToString());
			this.AddDefaultHeader("KRISP_HEADER_ID", InstallationID.ID);
		}
		...
		public void DoAsyncRequest<T>(RequestInfo requestInfo, Action<KrispSDKResponse<T>, object> callback)
		{
			...
			this.ExecuteAsync(restRequest, delegate(IRestResponse<KrispSDKResponse<T>> response)
			{
				try
				{

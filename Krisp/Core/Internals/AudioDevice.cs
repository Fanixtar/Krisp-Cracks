	...
	public class AudioDevice : DisposableBase, IAudioDevice, INotifyPropertyChanged, IDiagnosticsBase
	{
		...
		private void ReadProperties()
		{
			...
			try
			{
				...
				if (this.Kind == AudioDeviceKind.Microphone && this._enumeratorName == "USB" && DeviceLoginHelper.DeviceMode && (this._interfaceName.Contains("Jabra EVOLVE LINK") || this._interfaceName.Contains("Jabra EVOLVE 20")))
				{
					this._deviceContainerId = propertyStore.GetValue(PropertyKeys.PKEY_Devices_ContainerId);
					this.checkForHIDHeadset();
				}
			}
			catch (Exception ex)
			{

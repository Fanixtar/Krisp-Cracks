...
namespace Krisp.Core.Internals
{
	public class AccountManager : IAccountManager
	{
		...
		public void RecoverAfterSyncError()
		{
			if (!this.HasSecret)
			{
				new Task(new Action(this.CreateInstallation)).Start();
				return;
			}
			if (DeviceLoginHelper.DeviceMode && !this.HasAppToken)
			{
				new Task(new Action(this.DeviceLogin)).Start();
				return;
			}
			this.FetchUserProfileInfoAsync();
		}
		...
		private static Dictionary<string, AccountManagerErrorCode> ErrorCodeMap
		{
			get
			{
				...
				if ((dictionary = AccountManager._errorCodeMap) == null)
				{
					...
					dictionary2.Add("TEAM_NOT_FOUND", AccountManagerErrorCode.TEAM_NOT_FOUND);
					...
					dictionary2.Add("INSTALL_ID_MECHANISM_NOT_ENABLED", AccountManagerErrorCode.INSTALL_ID_MECHANISM_NOT_ENABLED);
				}
		...
		{
			UserProfileInfo userProfileInfo = AccountManager.Instance.UserProfileInfo;
			ProfileSettings profileSettings = ((userProfileInfo != null) ? userProfileInfo.settings : null);
			UpdateSetting updateSetting = ((profileSettings != null) ? profileSettings.update : null);
			return updateSetting != null && updateSetting.available && updateSetting.prevent_update != "on";
		}

		public static bool ShowContactSupport()
		{
			UserProfileInfo userProfileInfo = AccountManager.Instance.UserProfileInfo;
			ProfileSettings profileSettings = ((userProfileInfo != null) ? userProfileInfo.settings : null);
			BaseProfileSetting baseProfileSetting = ((profileSettings != null) ? profileSettings.contact_support : null);
			return baseProfileSetting == null || baseProfileSetting.available;
		}

		public static bool ShowReportProblem()
		{
			UserProfileInfo userProfileInfo = AccountManager.Instance.UserProfileInfo;
			ProfileSettings profileSettings = ((userProfileInfo != null) ? userProfileInfo.settings : null);
			BaseProfileSetting baseProfileSetting = ((profileSettings != null) ? profileSettings.report_problem : null);
			return baseProfileSetting == null || baseProfileSetting.available;
		}

	...
	public static class InstallationID
	{
		...
		public static string HWID
		{
			get
			{
				...
				try
				{
					if (DeviceLoginHelper.DeviceMode && DeviceLoginHelper.FQDNBasedInstallID)
					{
						text = CryptoHelper.ComputeSha256(SystemInfo.FQDN, InstallationID.HW_ID_BYTES_COUNT);
					}
					else
					{
		...
		{
			return instId.StartsWith(InstallationID.devicePrefix) || instId.StartsWith(InstallationID.deviceFqdnPrefix);
		}
		...
		private static string InsID
		{
			get
			{
				...
				try
				{
					...
					{
						string text2 = (DeviceLoginHelper.FQDNBasedInstallID ? SystemInfo.FQDN : InstallationID.HardwareIdentifier);
						string text3 = (DeviceLoginHelper.FQDNBasedInstallID ? InstallationID.deviceFqdnPrefix : InstallationID.devicePrefix);
						AppLocalCache.Instance.Set(InstallationID.INST_ID_CFG_KEY_NAME, text3 + CryptoHelper.ComputeSha256(text2, InstallationID.KIID_BYTES_COUNT - text3.Length / 2));
						text = AppLocalCache.Instance.Get(InstallationID.INST_ID_CFG_KEY_NAME);
					}
		...
		private static string deviceFqdnPrefix = "DEVICEQN";
	}
}

...
namespace Krisp.AppHelper
{
	internal class HttpDownloadClient : IDisposable
	{
		...
		public async Task<HttpAsyncDownloadResult> StartDownload()
		{
			...
			try
			{
				HttpResponseMessage httpResponseMessage = await this._httpClient.GetAsync(this._downloadUrl, HttpCompletionOption.ResponseHeadersRead, this._cancellationToken.Value);

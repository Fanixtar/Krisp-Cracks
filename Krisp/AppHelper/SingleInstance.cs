...
namespace Krisp.AppHelper
{
	public static class SingleInstance
	{
		...
		public static bool Start()
		{
			...
			if (!flag)
			{
				try
				{
					...
					if (array.Length != 1)
					{
						SentrySdk.CaptureMessage("Krisp process was detected which quited gracefully.", SentryLevel.Info);
					...
					if (!process.HasExited)
					{
						...
						SentrySdk.CaptureMessage("Found another instance of Krisp which was not responding. Killing that process and moving on.", SentryLevel.Warning);
		...
		private static bool KillProcess(Process process)
		{
			...
			{
				SentrySdk.CaptureMessage("Win32Exception thrown during process Kill. Error: " + ex.Message, SentryLevel.Warning);
				return false;
			}
			...
			{
				SentrySdk.CaptureMessage("NotSupportedException thrown during process Kill. Error: " + ex2.Message, SentryLevel.Error);
			}
			return true;
		}

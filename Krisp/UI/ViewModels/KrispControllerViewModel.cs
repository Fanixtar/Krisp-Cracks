...
namespace Krisp.UI.ViewModels
{
	public class KrispControllerViewModel : BindableBase
	{
		...
		public bool NCSwitch
		{
			get
			{
				return this._ncSwitch && !this.ForceDisabled;
			}
			set
			{
				if (this._ncSwitch != value)
				{
					...
					ncswitchEvent(this, this._ncSwitch && !this.ForceDisabled);
				}
			}
		}
		...
		public bool NCToggleState
		{
			get
			{
				...
				return flag && !this.ForceDisabled;
			}
			set
			{
				if (this._toggleState != value)
				{
					AnalyticsFactory.Instance.Report(AnalyticEventComposer.NCStateSwitchedEvent(this._kind == AudioDeviceKind.Speaker, value));
					this._logger.LogInfo(string.Format("NC toggle for {0} is {1}", (this._kind == AudioDeviceKind.Microphone) ? "microphone" : "speaker", value ? "enabled" : "disabled"));
		...
		private bool ForceDisabled
		{
			...
			set
			{
				...
				{
					AnalyticsFactory.Instance.Report(AnalyticEventComposer.NCStateSwitchedEvent(this._kind == AudioDeviceKind.Speaker, !value));
					this._logger.LogInfo(string.Format("NC toggle for {0} is {1}", (this._kind == AudioDeviceKind.Microphone) ? "microphone" : "speaker", value ? "disabled" : "enabled"));
		...
		{
			this._logger = LogWrapper.GetLogger("KrispControllerViewModel");
			this._kind = kind;
		...
		private void _watcherHolder_SessionConnected(object sender, IAppInfo e)
		{
			...
			if (!AppSettingsHelper.Instance.IgnoreList.Contains(e.ExeName))
			{

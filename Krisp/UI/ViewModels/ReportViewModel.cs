	...
	public class ReportViewModel
	{
		...
		public ICommand SendReportCommand
		{
			get
			{
				...
				{
					AnalyticsFactory.Instance.Report(AnalyticEventComposer.ReportEvent(null));
		...
		public string GenerateReportFile(string selectedPath = null)
		{
			...
			try
			{
				...
				if (this.IncludeSysInfo)
				{
					...
					stringBuilder.AppendLine("fqdn: " + SystemInfo.FQDN).AppendLine();
		...
		private bool ReportBug()
		{
			...
			try
			{
				...
				if (text2 != null)
				{
					...
					this._logger.LogWarning(string.Format("Failed to UploadDebugInfo. status: {0}, msg: {1}", restResponse.StatusCode, restResponse.ErrorMessage));
					string text3 = string.Format("status: {0}, msg: {1}", restResponse.StatusCode, restResponse.ErrorMessage);
					AnalyticsFactory.Instance.Report(AnalyticEventComposer.ReportEvent(text3));
					return false;
				}
			}
			catch (Exception ex)

...
namespace Krisp.App
{
	public partial class KrispApp : Application
	{
		...
		protected override void OnStartup(StartupEventArgs se)
		{
			...
			if (se.Args.Length != 0)
			{
				ParserResultExtensions.WithParsed<CmdOptions>(Parser.Default.ParseArguments<CmdOptions>(se.Args), delegate(CmdOptions o)
				{
					IsSilent = o.Silent;
			...
			using (SentrySdk.Init("https://d490cb1fc1fb41cd87fb7c8345a7674c@sentry.io/1430452"))
			{
				try
				{
					SentrySdk.ConfigureScope(delegate(Scope scope)
					{
						...
						BaseScopeExtensions.SetTag(scope, "installation_id", InstallationID.ID);
		...
		private static void InitializeLogger()
		{
			...
			if (DeviceLoginHelper.DeviceMode && level > Traces.Level.DEBUG)
			{
				level = Traces.Level.DEBUG;
			}
